...
export class Issue {
  ...
  /** 그 날 이 선택지의 성공 확률. */
  getSuccessRate(option: IssueOption, day: number): number {
    return getSuccessRateForDay(option.success, option.kind, day)
  }
   ...
   */
  isAffordable(option: IssueOption, player: PlayerStatus): boolean {
   ...
   */
  resolve(option: IssueOption, context: ResolveContext): ResolveOutcome {
    const { player, day, rng } = context
    ...
    if (!this.isAffordable(option, player)) return miss
    ...
    const solved = rng.rollChance(this.getSuccessRate(option, day))
   ...
   * 단위를 섞으면 조용히 어긋납니다.
   */
  onNeglect(_minutes: number, pressure: NeglectPressure, _context: ResolveContext): void {
