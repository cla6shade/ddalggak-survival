Reference image (most important requirement, overrides everything else): the image you opened at `{{reference}}` is the SAME CHARACTER this sheet must show, already drawn. It is not a mood board and not a style hint — it is the character sheet. Reproduce that character and change ONLY the viewing angle and the pose described below.
...
Anything the reference already settles is settled. Do not redesign it, do not restyle it, do not re-proportion it, and do not improve it. If the reference and the description below disagree about how the character looks, the reference wins; the description only decides the pose and the viewing angle.
